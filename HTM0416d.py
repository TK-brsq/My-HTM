...
class Segment:
    ...
    def add_synapse(self, target: Tuple[int, int], weight: float):
    ...
    def calc_overlap(self, active_state_idx):
...
class TemporalPooling:
    ...
    def __post_init__(self):
        self.active_state = np.zeros((len(self.columns), self.num_cells), dtype=bool)
        self.predictive_state = np.zeros((len(self.columns), self.num_cells), dtype=bool)
